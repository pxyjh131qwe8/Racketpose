...
import csv
import os
os.environ['CUDA_VISIBLE_DEVICES'] = "1"  # 在导入 torch 前设置可见 GPU
...
def _build_coco_structs(preds_accum, gts_accum, num_classes: int):
    """
    把收集到的 preds/gts 列表转成 COCO 的 images/annotations/categories 以及 dt 列表
    preds_accum: List[{"image_id":int, "boxes":Tensor[N,4](xyxy pix), "scores":Tensor[N], "labels":Tensor[N]}]
    gts_accum  : List[{"image_id":int, "boxes":Tensor[M,4](xyxy pix), "labels":Tensor[M], "size":(H,W)}]
    """
    def _xyxy_to_xywh(xyxy: torch.Tensor):
        if xyxy.numel() == 0:
            return xyxy
    ...
    for gt in gts_accum:
        ...
        # 记录 images（必须包含每张图片一次；width/height 必须是 int 且 >0）
        if img_id not in seen_img_ids:
            H, W = gt.get("size", (None, None))
            if (H is None) or (W is None) or (H == 0) or (W == 0):
                # 兜底：从 boxes 粗略估出尺寸（不可靠但能过 API）
                if gt["boxes"].numel():
                    W = int(max(1, float(gt["boxes"][:, [0, 2]].max())))
                    H = int(max(1, float(gt["boxes"][:, [1, 3]].max())))
                else:
                    # 实在没有，就给个占位，但建议你在 test 循环里传真实 size
                    W, H = 640, 640
...
def _coco_eval_from_accum(preds_accum, gts_accum, num_classes: int):
    ...
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".json", delete=False) as f_gt, \
        ...
        coco_dt = coco_gt.loadRes(f_dt.name)  # 空也可以
...
def test(args, params, model=None):
    ...
    if not model:
        ...
        # x-old 
        model = torch.load(f='/data1/wangqiurui/pxy/yolov11-new/multitask_weights/x/old/best.pt', map_location='cuda', weights_only=False) 
        # m-old 
        # model = torch.load(f='/root/autodl-tmp/yolov11/multitask_weights/m/old/best.pt', map_location='cuda', weights_only=False)
        # l-old 
        # model = torch.load(f='/root/autodl-tmp/yolov11/multitask_weights/l/old/best.pt', map_location='cuda', weights_only=False)
        # t-old 
        # model = torch.load(f='/root/autodl-tmp/yolov11/multitask_weights/t/old/best.pt', map_location='cuda', weights_only=False) 
        # s-old 
        # model = torch.load(f='/root/autodl-tmp/yolov11/multitask_weights/s/old/best.pt', map_location='cuda', weights_only=False)
        # n-old 
        # model = torch.load(f='/root/autodl-tmp/yolov11/multitask_weights/n/old/best.pt', map_location='cuda', weights_only=False)
        # x-new
        # model = torch.load(f='/data1/wangqiurui/pxy/yolov11-new/new_weights_multi/x/train-2025_11_25-183028/weights/best.pt', map_location='cuda', weights_only=False)
        # m-new
        # model = torch.load(f='/data1/wangqiurui/pxy/yolov11-new/new_weights_multi/m/train-2025_12_07-154926/weights/best.pt', map_location='cuda', weights_only=False)
        # l-new
        # model = torch.load(f='/data1/wangqiurui/pxy/yolov11-new/new_weights_multi/l/train-2025_11_30-144040/weights/best.pt', map_location='cuda', weights_only=False)
    ...
    for samples, regs, clss, boxes_list, labels_list in p_bar:
        ...
        for i, output in enumerate(outputs):
            ...
            if boxes_list[i].numel() == 0:
                ...
                # 但 COCO 也要记录空 GT
                Himg, Wimg = samples.shape[2], samples.shape[3]
                gts_accum.append({
            ...
            gts_accum.append({
                "image_id": global_img_idx,
                "boxes": (box * scale).clone(),             # 像素
                "labels": cls.squeeze(1).long().clone(),
                "size": (Himg, Wimg)
            })
            
            # COCO det：output [:, :6] 格式通常是 xyxy + conf + cls
            if output is not None and output.numel():
                # output: [N,6] -> x1,y1,x2,y2,conf,cls （你 util.nms 默认这个）
                preds_accum.append({
                    "image_id": global_img_idx,
        ...
        if isinstance(out_all, (tuple, list)) and len(out_all) == 2 and isinstance(out_all[1], dict):
            ...
            if (angle_norm is not None) and (dist_log is not None):
                ...
                dist_pred       = _expm1_clamp(dist_log.squeeze(-1).float())             # [B]
...
def main():
    ...
    parser.add_argument('--data-root', default="/data1/wangqiurui/pxy/Dataset/racketpose/data", type=str)   # <<< 新增
