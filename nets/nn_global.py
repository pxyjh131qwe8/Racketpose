...
class PoseGlobalNet(torch.nn.Module):
    ...
    def __init__(
        ...
        # 把 FPN 输出的 p3/p4/p5 投影到同一维度 roi_ch
        ...
        # 可选：给全局特征也加一点卷积“塔”（与 ROI head 风格接近）
    ...
    def _fpn_feats(self, x: torch.Tensor):
        # DarkNet -> DarkFPN 输出通道分别是 width[3], width[4], width[5]
    ...
    def _global_vec(self, p3: torch.Tensor, p4: torch.Tensor, p5: torch.Tensor) -> torch.Tensor:
        # 统一：先过一个小 conv tower 再 GAP（让特征更“可回归”）
