...
# def roi_pose_v11_x(num_classes: int, img_size: int = 640, roi_ch: int = 256):
#     csp = [True, True]
#     depth = [2, 2, 2, 2, 2, 2]
#     width = [3, 96, 192, 384, 768, 768]
#     return PoseROINet(width, depth, csp, num_classes, img_size=img_size, roi_ch=roi_ch)
...
class PoseROIHead(torch.nn.Module):
    """
    支持融合全局特征：
      - roi_feat -> conv + GAP -> roi_vec [B, in_ch]
      - global_vec [B, in_ch]（可选）
      - concat -> MLP -> outputs
    """
...
class PoseROINet(torch.nn.Module):
    ...
    def __init__(
        ...
        mlp_hidden: int = 512,
        dropout: float = 0.0,
        use_global: bool = True,     #  新增：是否融合全局特征
        global_from: str = "p5",      #  新增：全局特征来源：p5 或 p345
    ):
        super().__init__()
        self.nc = int(num_classes)
        self.img_size = int(img_size)
    ...
    def _global_vec(self, feats: dict) -> torch.Tensor:
        ...
        输出全局向量 [B, C]
        - p5: 仅用最深层
        - p345: 三层 GAP 后求平均（或可改 concat，但会改维度）
        """
    ...
    def forward(self, images: torch.Tensor, boxes: list):
        """
        images: [B,3,H,W]
        boxes : list length B, each tensor [K,4] in xyxy pixel coords（建议 K=1）
        """
        B, _, H, W = images.shape
        feats = self._fpn_feats(images)
        image_shapes = [(H, W)] * B
