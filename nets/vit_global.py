...
# ---------------- Head: 尽量保持和原 PoseROIHead 一致 ----------------
class PoseGlobalHead(nn.Module):
    """
    对齐你原本 PoseROIHead 的风格：
      global_vec [B, C]
         -> Linear -> SiLU -> Dropout(optional)
         -> Linear -> SiLU -> Dropout(optional)
         -> fc_center / fc_normal / fc_cls
    """
...
def vit_global_pose_b(
    ...
    return ViTGlobalPose(
        ...
        dropout=0.1,
        emb_dropout=0.1,
        head_hidden=512,   # 对齐原 head hidden
        head_dropout=0.0,  # 你原版默认也是 0
    )
