...
#         # ======= 关键改动：回归维度从 bbox(DFL) -> 6 维 pose =======
#         self.reg_dim = 6                      # center(3) + normal(3)
#         self.no = self.nc + self.reg_dim      # 输出通道数
#         self.stride = torch.zeros(self.nl)

#         reg_ch = max(64, filters[0] // 4)
#         cls_ch = max(80, filters[0], self.nc)

#         # ======= 沿用原来的回归塔结构，只改最后输出层 out_channels =======
#         self.box = torch.nn.ModuleList(
#             torch.nn.Sequential(
#                 Conv(x, reg_ch, torch.nn.SiLU(), k=3, p=1),
#                 Conv(reg_ch, reg_ch, torch.nn.SiLU(), k=3, p=1),
#                 torch.nn.Conv2d(reg_ch, out_channels=self.reg_dim, kernel_size=1)  # <-- 原来是 4*self.ch
#             ) for x in filters
#         )
...
#                 Conv(x, cls_ch, torch.nn.SiLU()),
#                 Conv(cls_ch, cls_ch, torch.nn.SiLU(), k=3, p=1, g=cls_ch),
#                 Conv(cls_ch, cls_ch, torch.nn.SiLU()),
#                 torch.nn.Conv2d(cls_ch, out_channels=self.nc, kernel_size=1)
#             ) for x in filters
#         )
...
#             x[i] = torch.cat((box(x[i]), cls(x[i])), dim=1)  # [B, 6+nc, H, W]
...
#         B = x[0].shape[0]
#         x = torch.cat([i.view(B, self.no, -1) for i in x], dim=2)  # [B, 6+nc, N]

#         reg, cls = x.split((self.reg_dim, self.nc), dim=1)         # reg:[B,6,N], cls:[B,nc,N]

#         # 拆成 center + normal
#         center = reg[:, 0:3, :]                                    # [B,3,N]
#         normal = reg[:, 3:6, :]                                    # [B,3,N]

#         # normal 建议做 [-1,1] 限幅 + 单位化（更稳定）
#         normal = torch.tanh(normal)
#         normal = F.normalize(normal, dim=1, eps=1e-6)

#         return torch.cat((center, normal, cls.sigmoid()), dim=1)   # [B, 6+nc, N]

#     def initialize_biases(self):
#         # 回归头 bias -> 0（原 bbox 的 1.0 不适用）
#         for box in self.box:
#             torch.nn.init.zeros_(box[-1].bias)

#         # cls bias 沿用 YOLO 初始化
#         for cls, s in zip(self.cls, self.stride):
...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=()):
        ...
        # ======= 回归维度：center(3) + normal(3) =======
        self.reg_dim = 6
        self.no = self.nc + self.reg_dim
        self.stride = torch.zeros(self.nl)
        ...
        self.box = torch.nn.ModuleList(
            torch.nn.Sequential(
                Conv(x, reg_ch, torch.nn.SiLU(), k=3, p=1),
                Conv(reg_ch, reg_ch, torch.nn.SiLU(), k=3, p=1),
                torch.nn.Conv2d(reg_ch, out_channels=self.reg_dim, kernel_size=1)
            ) for x in filters
        )
        ...
        # ======= 新增：center 标准化统计量（默认不标准化：mean=0,std=1） =======
        self.register_buffer("center_mean", torch.zeros(1, 3, 1))  # [1,3,1]
        self.register_buffer("center_std",  torch.ones(1, 3, 1))   # [1,3,1]
        self.denorm_inference = True  # 推理时把 center_norm 还原成米

    @torch.no_grad()
    def set_center_stats(self, mean_xyz, std_xyz, eps=1e-6, denorm_inference=True):
        """
        mean_xyz/std_xyz: array-like of shape (3,), 以米为单位统计出来的均值/标准差
        """
        m = torch.as_tensor(mean_xyz, dtype=torch.float32).view(1, 3, 1)
        s = torch.as_tensor(std_xyz,  dtype=torch.float32).view(1, 3, 1).clamp_min(eps)
    ...
    def forward(self, x):
        ...
        # 训练阶段：保持原样返回 list（其中 center 通道表示 center_norm）
        if self.training:
            return x
        ...
        center_norm = reg[:, 0:3, :]                               # [B,3,N]  —— 标准化空间
        normal = reg[:, 3:6, :]                                    # [B,3,N]
        ...
        if self.denorm_inference:
            center = center_norm * self.center_std + self.center_mean
        else:
            center = center_norm
