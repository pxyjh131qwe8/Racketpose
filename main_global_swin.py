...
from nets import nn_swin as pose_nn
...
class PoseROILoss(torch.nn.Module):
    """
    这里名字沿用你原来的 PoseROILoss，避免动太多别的代码。
    实际上现在用于 global ViT 输出：
      pred["cls_logits"], pred["center_norm"], pred["normal"]
    """
...
def train(args, params):
    ...
    if args.model_size == "b":
        model = pose_nn.swin_global_pose_b(
            num_classes=nc,
            img_size=args.input_size,
        ).to(device)
    elif args.model_size == "s":
        model = pose_nn.swin_global_pose_s(
            num_classes=nc,
            img_size=args.input_size,
        ).to(device)
    elif args.model_size == "t":
        model = pose_nn.swin_global_pose_t(
            num_classes=nc,
            img_size=args.input_size,
        ).to(device)    
    else:
        raise ValueError(f"Unknown model_size={args.model_size}, expected 'b' or 'l'")
    ...
    _unwrap(model).set_center_stats(
        train_set.center_mean.tolist(),
        train_set.center_std.tolist(),
        denorm_inference=True
    )
    ...
    with open(step_csv, "w", newline="") as log:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                # 如果你后面想切到 EMA eval，这里改回 ema.ema 即可
                eval_model = model
...
def test(args, params):
    ...
    if hasattr(model, "center_mean") and hasattr(model, "center_std"):
        stats = (
            model.center_mean.detach().cpu().view(3),
            model.center_std.detach().cpu().view(3)
        )
...
def main():
    ...
    # model
    parser.add_argument("--model-size", type=str, default="b", choices=["b", "s", "t"])
    ...
    run_root = os.path.join("runs", f"pose-vit-global-{ts}")
    if args.local_rank == 0:
