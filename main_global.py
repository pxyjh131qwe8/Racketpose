# main_pose_global.py   (你也可以继续叫 main_pose_roi.py，只是内容已变成 global-only)
import copy
import csv
import os
...
from nets import nn_global as pose_nn   # 这里应包含 global_pose_v11_x
...
def eval_pose_global(
    ...
    for imgs, targets in tqdm.tqdm(loader, desc="Eval", leave=False):
        ...
        ang = torch.acos(dot) * (180.0 / math.pi)  # [0,180]，已是“绝对角度”
...
def train(args, params):
    ...
    # 写入 center stats：保证 eval 输出 meters
...
def test(args, params):
    ...
    # 用模型里存的 stats 让 dataset 的 center_norm 对齐训练
    stats = None
...
def main():
    ...
    parser.add_argument("--roi-ch", type=int, default=256)  # 这里作为投影通道/全局通道
