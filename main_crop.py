...
from utils.crop_utils import crop_from_detections, crop_from_annotations
...
def main():
    ...
    parser.add_argument('--crop', action='store_true', help='运行从检测结果裁剪分支')
    ...
    parser.add_argument('--crop-dst-root', type=str, default="/root/autodl-tmp/Dataset/racketpose/cropx1.5/imgs", help='裁剪输出根目录（将按原相对路径创建子目录）')
    parser.add_argument('--crop-mode', type=str, default='more', choices=['bbox','more'], help='裁剪模式')
    parser.add_argument('--scale', type=float, default=1.5, help='裁剪后调整的尺寸')
    parser.add_argument('--crop-conf-thres', type=float, default=0.2, help='裁剪时的最小置信度阈值（过滤框）')
    
    # —— 新增：按 VOC 标注裁剪 ——
    parser.add_argument('--crop-ann', action='store_true', help='按VOC标注框裁剪（不依赖检测）')
    parser.add_argument('--voc_boxes_root', type=str, default="/root/autodl-tmp/Dataset/racketpose/data/boxes",
                        help='VOC XML 根目录（默认 data_root/boxes ）')
    parser.add_argument('--ann_scale', type=float, default=1.5,
                        help='按中心等比扩展倍数（=1不变，>1放大，<1缩小）')  # 确保是 float
    parser.add_argument('--ann_src_root', type=str, default="/root/autodl-tmp/Dataset/racketpose/data/imgs", help='原图根目录（与 detect-src 一致）')
    parser.add_argument('--crop_ann_root', type=str, default="/root/autodl-tmp/Dataset/racketpose/cropannx1.5/imgs", help='裁剪输出根目录（将按原相对路径创建子目录）')
